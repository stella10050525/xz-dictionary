...
async function displayWordWithPredefinedExample(word) {
    ...
    const html = '<div class="word-card">' +
        '<div class="word-content-wrapper"><div class="word-main-content">' +
        '<div class="word-header"><div class="word-title">' +
        '<h2>' + word + '</h2>' +
        ...
        (isInVocabulary(word) ? '⭐' : '☆') + '</button></div></div>' +
        '<div class="meanings"><div class="meaning-item">' +
        '<div class="definition">📖 ' + meaning + '</div></div></div>' +
        '<div class="examples"><h3>📝 例句 <span class="xz-tag">专属例句</span></h3>' +
        '<div class="example-item"><div class="example-en">' +
        '<span class="example-text">' + wordData.en + '</span>' +
        '<button class="btn-speak-example" onclick="speakText(\'' + wordData.en.replace(/'/g, "\\'") + '\')">🔊</button>' +
        '</div><div class="example-zh">' + wordData.zh + '</div></div></div>' +
        renderWordImageSection(wordImage, word) +
        '</div></div></div>';
...
async function displayWordFromAPI(word, data) {
    ...
    const html = '<div class="word-card"><div class="word-content-wrapper"><div class="word-main-content">' +
        '<div class="word-header"><div class="word-title"><h2>' + word + '</h2>' +
        (phonetic ? '<span class="phonetic">' + phonetic + '</span>' : '') +
        '</div><div class="word-actions">' +
        '<button class="btn-icon" onclick="speakWord(\'' + word + '\')" title="发音">🔊</button>' +
        '<button class="btn-icon ' + (isInVocabulary(word) ? 'saved' : '') + '" ' +
        ...
        '<span style="color: var(--warning); font-size: 0.9rem;">(仅英文释义)</span></div></div></div>' +
        ...
        '或者去 <a href="#" onclick="switchTab(\'custom\'); return false;" style="color: var(--primary-color);">自定义添加</a> 哦~</p></div>' +
        '<div class="examples"><h3>📝 例句</h3>' +
        '<div class="no-example-notice" style="background: #f8f9fa; padding: 20px; border-radius: 8px; text-align: center;">' +
        '<p style="color: var(--text-light); margin-bottom: 15px;">这个词的专属例句在路上了～</p>' +
        '<ul style="list-style: none; padding: 0; margin: 10px 0; color: var(--text-light); text-align: left; max-width: 400px; margin-left: auto; margin-right: auto;">' +
...
function displayErrorNotFound(word) {
    resultArea.innerHTML = '<div class="word-card">' +
...
// 辅助函数：渲染图片区域
function renderWordImageSection(wordImage, word) {
    if (wordImage && wordImage.type === 'images' && wordImage.content && wordImage.content.length > 0) {
        return '<div class="word-image-section">' +
            wordImage.content.map(img => 
                '<img src="' + img.data + '" alt="' + word + '" class="word-image" style="max-width: 100%; border-radius: 8px; margin-top: 15px;" />'
            ).join('') +
            '</div>';
    } else {
        return '<div class="upload-section" style="margin-top: 20px; text-align: center;">' +
            '<button onclick="uploadWordImage(\'' + word + '\')" class="btn-secondary">📸 上传图片</button>' +
            '</div>';
    }
}

console.log('✅ XZ Dictionary 查词逻辑优化（方案C）已加载');
